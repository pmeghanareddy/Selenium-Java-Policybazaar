...
import com.BasePackage.ExcelUtility;
...
//		driver.findElement(By.xpath("//input[@class='date_to']")).click();
    ...
    public void enterPersonDetails() throws InterruptedException, IOException
    {
    	String[] input = eu.readFromExcel();
		Select s = new Select(driver.findElement(gender));
		s.selectByVisibleText("Ms.");
		
		driver.findElement(travelName).sendKeys(input[0]);
		driver.findElement(travelmobile).sendKeys(input[1]);
		driver.findElement(travelemail).sendKeys(input[2]);
    ...
    public void getLowestPrices() throws InterruptedException
    {
		...
		Thread.sleep(1000);
	//	 List<WebElement> elements = driver.findElements(By.xpath("//button/span[@class='ng-binding ng-scope']"));
	//	 for(int i=0;i<3;i++)
	//	 {
